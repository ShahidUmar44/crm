...
import { View, StyleSheet, Text, ScrollView, Image, Pressable, Modal, TouchableOpacity } from 'react-native';
...
const NewJobPresenter = ({ customers, users, addNewJob }) => {
  ...
  return (
    ...
    <View
      ...
      <View style={styles.headerView}>
        ...
        />
        <Text style={styles.header}>New Job</Text>
      </View>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: spacing.SCALE_10 }}>
          <Schedule
            defaultStartTime={moment().hours(9).minutes(0).format('hh:mm A')}
            defaultEndTime={moment().hours(11).minutes(0).format('hh:mm A')}
            defaultStartDate={moment().format('MM/DD/YYYY')}
        ...
        <DispatchModal data={users} setSelectUser={setSelectUser} />
        <View
          style={{
            marginTop: spacing.SCALE_20,
          }}>
        ...
        <View style={{ paddingBottom: 200 }}>
          <BigButton marginBottom={spacing.SCALE_20} width={'60%'} onPress={onSubmit}>
            <Text style={{ color: colors.yellow400, fontSize: typography.FONT_SIZE_16, fontWeight: 'bold' }}>Save</Text>
          </BigButton>
        </View>
      </ScrollView>
      <LeaveScreenModal
...
export default NewJobPresenter;
